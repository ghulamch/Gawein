...
@section('sidebar_menu')
    ...
    <li><a href="{{ route('employer.settings.index') }}" class="sidebar-link active"><i class="fa-solid fa-gear w-5 text-center"></i> Pengaturan</a></li>
@endsection
    ...
    <div class="glass-panel p-8 md:p-10 rounded-[2rem] shadow-sm">
        ...
        <div class="flex items-center gap-4 mb-2">
            ...
            <div>
                ...
                <p class="text-sm text-slate-500 font-medium">Atur kata sandi dan email untuk keamanan akun perusahaan Anda.</p>
            </div>
        </div>
        ...
        <div class="mt-10 space-y-8">
            ...
            <div class="bg-slate-50 rounded-2xl p-6 border border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div class="flex-1">
                    ...
                    <div class="text-sm text-slate-500 mb-3">Email ini digunakan sebagai kontak utama akun (Tidak dapat diubah).</div>
            ...
            <form action="{{ route('employer.settings.password') }}" method="POST" class="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm relative overflow-hidden">
                ...
                <div class="text-sm text-slate-500 mb-6">Pastikan akun Anda aman dengan kata sandi yang kuat (minimal 8 karakter).</div>
            ...
            <div class="mt-12 pt-8 border-t border-slate-100">
                ...
                <div class="p-6 border border-rose-200 bg-rose-50/50 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-6">
                    <div>
                        ...
                        <div class="text-sm text-slate-600 max-w-md leading-relaxed">Tindakan ini tidak dapat dibatalkan. Semua data lowongan, profil perusahaan, serta riwayat kandidat Anda akan dihapus secara permanen dari sistem.</div>
                    </div>
                    
                    <form action="{{ route('employer.settings.destroy') }}" method="POST" id="form-delete-account" class="w-full md:w-auto shrink-0">
                        @csrf
                        @method('DELETE')
...
<script>
    ...
    function confirmDelete() {
        Swal.fire({
            ...
            html: "<p class='text-slate-500 font-medium text-sm mt-2'>Tindakan ini <b>tidak dapat dibatalkan</b>. Semua data perusahaan dan lowongan Anda akan hilang selamanya.</p>",
            icon: 'warning',
